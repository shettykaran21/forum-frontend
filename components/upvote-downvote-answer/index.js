...
const UpvoteDownvoteAnswer = ({ data, setData }) => {
  ...
  const { slug: questionId } = router.query
  ...
  const handleVote = async (vote) => {
    ...
    if (existingVoteValue === 1 || existingVoteValue === -1) {
      try {
        const { data } = await api.put(`/votes/unvote/${questionId}/${_id}`)
        setData(data.data)
      } catch (err) {
        console.log(err)
      }
      ...
      try {
        const { data } = await api.put(
          `/votes/${endPoint}vote/${questionId}/${_id}`
        )
        setData(data.data)
      } catch (err) {
        console.log(err)
